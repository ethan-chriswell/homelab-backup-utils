...
function submit() {
  if (file.value && selectedServiceId.value) {
    emit('upload', selectedServiceId.value, file.value)
  }
}
...
<template>
  ...
  >
    ...
    <div class="relative w-full max-w-md rounded-2xl bg-zinc-900 border border-zinc-800 shadow-2xl p-6">
      ...
      <!-- Service selector -->
      <div class="mb-4">
        <label class="block text-sm text-zinc-300 mb-1.5">Restore to service</label>
        <select
        ...
        >
          <option v-if="!services.length" value="" disabled>No services configured</option>
      ...
      <div class="flex gap-3 mt-6">
        ...
        <button
          class="flex-1 rounded-xl py-2.5 text-sm font-medium bg-violet-600 hover:bg-violet-500 text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          :disabled="!file || !selectedServiceId"
