...
const latest = computed(() => {
  if (!props.backups.length) return null
  return props.backups.slice().sort((a, b) => new Date(b.date) - new Date(a.date))[0]
})
...
<template>
  <div class="rounded-2xl bg-zinc-900 border border-zinc-800 p-6" data-testid="status-card">
    ...
    <div v-else-if="latest" data-testid="last-backup-time">
      <p class="text-2xl font-semibold text-zinc-100">{{ relativeTime(latest.date) }}</p>
      <p class="text-sm text-zinc-500 mt-1">{{ formatDate(latest.date) }}</p>
    </div>
